-- Idempotent seed: core roles, permission catalogue, role_permissions, helpers, RLS policies.
-- Mirrors sql/authentication_module.sql (roles) + sql/permissions_module.sql (permissions layer).
-- user_permissions: no rows seeded here (per-user overrides; requires auth user UUIDs).

create extension if not exists pgcrypto;

insert into public.roles (key, name, description, is_system) values
  ('call_center_agent', 'Call Center Agent', 'Submit and manage own leads', true),
  ('call_center_admin', 'Call Center Admin', 'Manage centre operations and agents', true),
  ('sales_manager', 'Sales Manager', 'Manage sales operations and assignments', true),
  ('sales_agent_licensed', 'Sales Agent (Licensed)', 'Licensed sales agent', true),
  ('sales_agent_unlicensed', 'Sales Agent (Unlicensed)', 'Unlicensed sales agent', true),
  ('system_admin', 'System Administrator', 'Full platform access', true),
  ('hr', 'HR', 'User lifecycle and people operations', true),
  ('accounting', 'Accounting', 'Commission and finance visibility', true),
  (
    'publisher_manager',
    'Publisher Manager',
...
-- Publisher Manager: no permission-key rows (UI gates by role); clear any stale mappings.
