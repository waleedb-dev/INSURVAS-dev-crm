...
export default function ColombianCenterPerformancePage() {
  ...
  const [colombianLeadVendors, setColombianLeadVendors] = useState<string[]>([]);
  const [hasColombianCenters, setHasColombianCenters] = useState<boolean | null>(null);
  ...
  const fetchThresholds = useCallback(async () => {
    console.log("[ColombianCenterPerformance] Fetching Colombian centers...");
    ...
    if (centersError) {
      console.error("[ColombianCenterPerformance] call_centers error:", centersError);
      setHasColombianCenters(false);
      setThresholds([]);
      setColombianLeadVendors([]);
      return;
    }

    const centerNames = (centersData || []).map((c) => c.name).filter(Boolean);
    if (centerNames.length === 0) {
      console.log("[ColombianCenterPerformance] No Colombian centers found.");
      setHasColombianCenters(false);
      setThresholds([]);
      setColombianLeadVendors([]);
      return;
    ...
    console.log("[ColombianCenterPerformance] Colombian centers:", centerNames);
    ...
    if (error) {
      console.error("[ColombianCenterPerformance] thresholds error:", error);
      setThresholds([]);
      setColombianLeadVendors([]);
    } else {
      const active = (data || []) as CenterThreshold[];
      console.log("[ColombianCenterPerformance] Thresholds fetched:", active.length, "active centers");
      console.log(
        "[ColombianCenterPerformance] Center names:",
        active.map((c) => ({ name: c.center_name, lead_vendor: c.lead_vendor, active: c.is_active }))
      );
      setThresholds(active);
      setColombianLeadVendors([...new Set(active.map((t) => t.lead_vendor))]);
      setHasColombianCenters(true);
    }
  }, [supabase]);
  ...
  const fetchData = useCallback(
    async (start: Date, end: Date) => {
      ...
      let query = supabase
        ...
        .select("*")
        .in("lead_vendor", colombianLeadVendors);
      query = query.gte("date", fromStr);
      query = query.lte("date", toStr);

      const { data, error } = await query;
      if (error) {
        console.error("[ColombianCenterPerformance] fetch error:", error);
        setRows([]);
      } else {
      ...
      if (prevError) {
        console.error("[ColombianCenterPerformance] prev fetch error:", prevError);
        setPrevRows([]);
      } else {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 28, paddingBottom: 24 }}>
      {/* Page Title */}
      <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#233217" }}>
        Colombian Centers Performance
      </h1>
      ...
      {loading ? (
        ...
        >
          <MinusCircle size={40} color="#647864" />
          <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#233217" }}>
            No Colombian centers configured
          </p>
          <p style={{ margin: 0, fontSize: 13, fontWeight: 500, color: T.textMuted }}>
            Add Colombian centers in the Call Centers settings to view performance data.
          </p>
        </div>
      ) : (
