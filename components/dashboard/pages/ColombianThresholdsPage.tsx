...
export default function ColombianThresholdsPage() {
  const supabase = useMemo(() => getSupabaseBrowserClient(), []);
  const [loading, setLoading] = useState(true);
  const [thresholds, setThresholds] = useState<CenterThreshold[]>([]);
  const [colombianCenterNames, setColombianCenterNames] = useState<string[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    // Step 1: Get Colombian centers from call_centers
    ...
    if (ccError) {
      console.error("[ColombianThresholds] call_centers fetch error:", ccError);
      setColombianCenterNames([]);
      setThresholds([]);
      setLoading(false);
      return;
    }

    const names = (ccData || []).map((c: CallCenter) => c.name);
    setColombianCenterNames(names);
    ...
    // Step 2: Get thresholds only for Colombian centers
    const { data, error } = await supabase
      .from("center_thresholds")
      .select("*")
      .in("center_name", names)
      .order("center_name", { ascending: true });

    if (error) {
      console.error("[ColombianThresholds] fetch error:", error);
      setThresholds([]);
    } else {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      console.error("[ColombianThresholds] save error:", error);
  ...
  const handleCreate = async () => {
    ...
    // Validate that center_name exists in call_centers with country = 'Colombia'
    const { data: ccData, error: ccError } = await supabase
      .from("call_centers")
      .select("name")
      .eq("name", newForm.center_name)
      .eq("country", "Colombia")
      .single();

    if (ccError || !ccData) {
      alert("Center Name must be a valid Colombian call center registered in the system.");
      return;
    }
    ...
    if (error) {
      console.error("[ColombianThresholds] create error:", error);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 28, paddingBottom: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: "#233217", margin: 0 }}>Colombian Centers Threshold</h2>
          <p style={{ margin: "4px 0 0", fontSize: 13, fontWeight: 500, color: "#647864" }}>
            Showing thresholds for Colombian call centers only
          </p>
        </div>
          ...
          {paginated.length === 0 && (
            <div style={{ padding: "60px 40px", textAlign: "center" }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: T.textMuted }}>
                {colombianCenterNames.length === 0
                  ? "No Colombian call centers found in the system"
