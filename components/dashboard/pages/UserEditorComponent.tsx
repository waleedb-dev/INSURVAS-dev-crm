...
export default function UserEditorComponent({ user, onClose, onSubmit }: UserEditorProps) {
  ...
  const handleFinalSubmit = () => {
    onSubmit({ firstName, lastName, email, phone, extension, roleId: selectedRoleId, centerId: isCallCenterRole ? selectedCenterId : null, permissions: Array.from(selectedPermissions) });
  };
      ...
      <div style={{ display: "flex", gap: 28, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 24 }}>
          <div style={{ backgroundColor: "#fff", borderRadius: "24px", padding: 0, boxShadow: "0 10px 25px -5px rgba(0,0,0,0.05)", border: `1.5px solid ${T.border}`, minHeight: 650, display: "flex", flexDirection: "column" }}>
              ...
              {activeTab === "User Info" && (
                <div style={{ animation: "fadeInUp 0.3s ease-out" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 32 }}>
                    <div><label style={labelStyle}>Given Name</label><input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="e.g. John" style={inputStyle} /></div>
                    <div><label style={labelStyle}>Family Name</label><input value={lastName} onChange={e => setLastName(e.target.value)} placeholder="e.g. Doe" style={inputStyle} /></div>
                    <div style={{ gridColumn: "span 2" }}><label style={labelStyle}>Email Address</label><input value={email} onChange={e => setEmail(e.target.value)} placeholder="john@example.com" style={inputStyle} /></div>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                    ...
                    {isCallCenterRole && (
                      <div style={{ animation: "fadeInDown 0.2s" }}><label style={labelStyle}>BPO Center</label>
            ...
            <div style={{ padding: "24px 40px", borderTop: `1.5px solid ${T.borderLight}`, display: "flex", justifyContent: "flex-end", gap: 12 }}>
              <button onClick={onClose} style={{ backgroundColor: "transparent", border: `1.5px solid ${T.border}`, borderRadius: T.radiusMd, padding: "10px 24px", fontSize: 13, fontWeight: 700, cursor: "pointer", color: T.textMuted }}>Cancel</button>
              {activeTab !== "User Info" && <button onClick={handleBack} style={{ backgroundColor: "#fff", border: `1.5px solid ${T.border}`, borderRadius: T.radiusMd, padding: "10px 24px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Back</button>}
              {activeTab === tabs[tabs.length - 1] ? (
                <button onClick={handleFinalSubmit} style={{ backgroundColor: T.blue, color: "#fff", border: "none", borderRadius: T.radiusMd, padding: "10px 32px", fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 12px ${T.blue}44` }}>Finish Setup</button>
              ) : (
                <button onClick={handleNext} style={{ backgroundColor: T.blue, color: "#fff", border: "none", borderRadius: T.radiusMd, padding: "10px 32px", fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 12px ${T.blue}44` }}>Next Step</button>
              )}
            </div>
          </div>
        </div>
