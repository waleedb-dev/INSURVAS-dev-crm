        ...
        <div style={{ width: 340, flexShrink: 0, display: "flex", flexDirection: "column", gap: 24 }}>
          <div style={{ backgroundColor: "#fff", borderRadius: "24px", padding: "32px 24px", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.05)", border: `1.5px solid ${T.border}`, textAlign: "center" }}>
            <div style={{ position: "relative", width: 90, height: 90, margin: "0 auto 20px" }}>
              ...
              >
                {isCreation ? "+" : fullName.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase()}
              </div>
            </div>
            ...
            {isCreation ? (
              ...
              <>
                <h2 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 800 }}>{fullName}</h2>
                <p style={{ margin: "0 0 8px", fontSize: 13, color: T.textMuted, fontWeight: 600 }}>
                  <code style={{ fontSize: 11 }}>lead_unique_id</code>: {String(display?.lead_unique_id ?? leadId ?? "—")}
                </p>
                <p style={{ margin: "0 0 16px", fontSize: 11, color: T.textMuted, wordBreak: "break-all" }}>
                  <code style={{ fontSize: 11 }}>id</code> (PK): {rowUuid ?? "—"}
                </p>
              </>
            )}
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
              {tags.length ? (
                tags.map((tag) => (
                  <span key={tag} style={{ backgroundColor: T.rowBg, color: T.textMid, fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: "8px" }}>
                    {tag}
                  </span>
                ))
              ) : (
                <span style={{ fontSize: 12, color: T.textMuted }}>No tags</span>
              )}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, borderTop: `1.5px solid ${T.borderLight}`, paddingTop: 24, textAlign: "left" }}>
              <div>
                <p style={{ margin: "0 0 4px", fontSize: 11, fontWeight: 700, color: T.textMuted, fontFamily: "ui-monospace, monospace" }}>stage</p>
                {isCreation ? (
                  <span style={{ backgroundColor: "#fef3c7", color: "#d97706", fontSize: 12, fontWeight: 800, padding: "2px 8px", borderRadius: "6px" }}>Draft</span>
                ) : isEditing && editDraft ? (
                  <select
                    value={String(editDraft.stage ?? "")}
                    onChange={(e) => patchDraft("stage", e.target.value)}
                    style={{ width: "100%", padding: 6, borderRadius: 6, border: `1px solid ${T.border}`, fontSize: 12, fontWeight: 700 }}
                  >
                  ...
                  <span style={{ backgroundColor: "#fef3c7", color: "#d97706", fontSize: 12, fontWeight: 800, padding: "2px 8px", borderRadius: "6px" }}>{String(display?.stage ?? "—")}</span>
                )}
              </div>
              <div>
                <p style={{ margin: "0 0 4px", fontSize: 11, fontWeight: 700, color: T.textMuted, fontFamily: "ui-monospace, monospace" }}>lead_value</p>
                {isCreation ? (
                  <input
                    type="number"
                    value={formData.premium}
                    onChange={(e) => setFormData({ ...formData, premium: Number(e.target.value) })}
                    style={{ width: "100%", padding: "4px 8px", borderRadius: 6, border: `1px solid ${T.border}`, fontSize: 14, fontWeight: 800 }}
                  />
                ) : isEditing && editDraft ? (
                  <input
                    type="number"
                  ...
                  <p style={{ margin: 0, fontSize: 16, fontWeight: 800 }}>${Number(display?.lead_value ?? 0).toLocaleString()}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: "#fff", borderRadius: "24px", padding: "24px", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.05)", border: `1.5px solid ${T.border}` }}>
            <h3 style={{ margin: "0 0 20px", fontSize: 17, fontWeight: 800 }}>
              Contact <span style={{ fontSize: 13, fontWeight: 600, color: T.textMuted }}>(phone)</span>
            </h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: "0 0 2px", fontSize: 11, fontWeight: 700, color: T.textMuted, textTransform: "uppercase" }}>Email</p>
                  <p style={{ margin: 0, fontSize: 12, color: T.textMuted, fontStyle: "italic" }}>Not stored on lead — use contact phone</p>
                </div>
              </div>
              <div style={{ display: "flex", gap: 12 }}>
                <div style={{ width: 32, height: 32, borderRadius: "10px", backgroundColor: T.rowBg, display: "flex", alignItems: "center", justifyContent: "center", color: T.textMid, flexShrink: 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: "0 0 2px", fontSize: 11, fontWeight: 700, color: T.textMuted, textTransform: "uppercase" }}>Phone</p>
                  {isCreation ? (
                    <input
                      value={formData.phone}
                      onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                      style={{ width: "100%", padding: "6px 8px", borderRadius: 6, border: `1px solid ${T.border}`, fontSize: 13, fontWeight: 700 }}
                    />
                  ) : isEditing && editDraft ? (
                    <input
                    ...
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: T.textDark, wordBreak: "break-word" }}>{String(display?.phone ?? "—")}</p>
                  )}
                </div>
              </div>
            </div>
